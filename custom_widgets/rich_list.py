...
class RichList(tk.Text):
    ...
    def filter(self, query, update_size):
        self._records, self._lines, self._match_results = self._searchable_records.filter(query)
    ...
    def _update_ui(self):
        ...
        if self._match_results is not None:
            for line_no, (line, line_annotation) in enumerate(zip(self._lines, self._match_results)):
                ...
                for tag_span in line_annotation.spans:
                    ...
                    self.tag_add("match", f'{widget_line_no}.{widget_span_start}', f'{widget_line_no}.{widget_span_end}')

        widget_sel_line_no = self._sel_line_no+1
        self.tag_add('arrow', f'{widget_sel_line_no}.0', f'{widget_sel_line_no}.1') 
        self.tag_add('selected_line', f'{widget_sel_line_no}.0', f'{widget_sel_line_no+1}.0')

...
class CmdFrame(tk.Frame):
    def __init__(self, master: tk.Tk, constructed_cmd: Cmd):
        ...
        self._cmd.custom_bind(MultiSegEntryEvent.ActiveSegFinished, self._on_active_seg_finished)
        self._cmd.custom_bind(MultiSegEntryEvent.LastSegActivated, self._on_last_seg_activated)
    ...
    def _update_help(self):
        file_path, line_no, help_message, para_start, para_end = self._constructed_cmd.get_help()
    ...
    def _on_describe_op(self, ev):
        ...
        line_no = int(line_no)
        launcher.open_with_vscode(app_common.PROJ_NAME, file_path, 
            functools.partial(package_config.launcher.open_folder_with_vscode, app_common.SOURCE_CODE_ROOT),
            line_no
        )
...
class App(tk.Tk):
    def __init__(self):
        ...
        self._constructed_cmd = Cmd(package_manager.build_op_list(package_config), package_manager)
        self._cmd_frame = CmdFrame(self, self._constructed_cmd)

    def invoke_cmd_frame(self, func: Callable = None, args: List = [], is_execute=False):
        ...
        if func is not None:
            ...
            input_cmd_segs = [self._constructed_cmd.get_op().name] + [str(seg) for seg in args]
        else:

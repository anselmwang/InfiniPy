...
root_node = SpaceKeyNode(" ", "root", [
    ...
    SpaceKeyNode("f", "File", [
        ...
        Leaf("o", "Open clipboard file path", launcher.open_clipboard_path_in_scratch_workspace, [])
    ]),
    SpaceKeyNode("d", "Directory", []),
    SpaceKeyNode("u", "URL", []),
    SpaceKeyNode("n", "OneNote Page", []),
    SpaceKeyNode("c", "Command", [
        Leaf("c", "Calculator", fun.calc)
    ]),
    SpaceKeyNode("D", f"{PROJ_NAME} Develop", [
        Leaf("p", "open package config", launcher.open_file_in_project_develop_environment, [SOURCE_CODE_ROOT, package_config.__file__]),
        Leaf("s", "open Space Key config", launcher.open_file_in_project_develop_environment, [SOURCE_CODE_ROOT, space_key_config_path]),
        Leaf("h", f"Goto to project home folder", launcher.goto_dir, [PROJ_HOME])
    ]),
], None)
...
url_table = {
    ...
    "t": ["Translate", "https://www.google.com/search?q=translate+english+to+chinese&oq=translate+"],
}

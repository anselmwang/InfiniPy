...
def build_function_to_invoke_cmd_frame(func: Callable = None, args: List = [], is_execute=False):
    def inner():
        invoke_cmd_frame(func, args, is_execute)
    return inner
    
class Leaf(SpaceKeyNode):
    def __init__(self, key:str, comment: str, cmd_func: Callable = None, args=[], is_execute=True):

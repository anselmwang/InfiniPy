...
package_manager.register_type_searcher_factory(str, lambda: StrSearcher(["candidate1", "candidate2"]))
def concat(a: str, b: str):
    """concat two string"""
    return a + b

op_table = [
    Op("test.concat", concat.__doc__, concat)
]
...
def test_filter():
    ...
    filtered_records, rendered_filtered_lines, filtered_match_results = searcher.filter("edge")
    assert filtered_records == [symbols[1]]
    assert len(rendered_filtered_lines) == 1
    assert rendered_filtered_lines[0].replace(" ", "") == 'goto_edge  desc: goto edge'.replace(" ", "")

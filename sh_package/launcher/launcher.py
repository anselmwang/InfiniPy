...
def open_with_vscode(
    vscode_title: str, file_path: str, func_to_start_vscode: Callable = None, line_no=0
):
    smart_activate(
        WindowFilter(proc_name="Code.exe", title=vscode_title), func_to_start_vscode
    )
    subprocess.run(["code", "-r", "-g", f"{file_path}:{line_no}"], shell=True)


def open_with_vscode_using_clipboard_path(
    vscode_title: str, func_to_start_vscode: Callable = None
):
    ...
    if len(windows) > 1:
        ...
        if func_to_start is not None:
            ...
            for i in range(max_retry):
                ...
                raise Exception(
                    f"Fail to find window {window_filter} after {max_retry} retries."
                )
        else:
...
def open_file_in_project_develop_environment(proj_folder, file_path):
    ...
    return open_with_vscode(
        proj_name, file_path, functools.partial(open_folder_with_vscode, proj_folder)
    )
...
def open_scratch_workspace():
    OPEN_SCRATCH_WORKSPACE = functools.partial(
        open_folder_with_vscode, SCRATCH_WORKSPACE_ROOT
    )
...
def open_clipboard_path_in_scratch_workspace():
    ...
    open_with_vscode_using_clipboard_path(
        SCRATCH_WORKSPACE_NAME, OPEN_SCRATCH_WORKSPACE
    )

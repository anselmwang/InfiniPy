...
def get_function_signature(f, cur_para_no=None) -> Union[str, Tuple[str, int, int]]:
    ...
    if cur_para_no is not None:
        assert cur_para_no < len(signature.parameters), f"cur_para_no {cur_para_no} is larger than function parameter count {len(signature.parameters)}"
...
class Cmd:
    ...
    def get_help(self):
        cmd_segs = self._cmd
        file_path, line_no, help_message, para_start, para_end = None, None, None, None, None
        if len(cmd_segs) != 0:
            ...
            if cur_para_no == get_function_parameter_count(op):
                ...
                help_message, para_start, para_end = get_function_signature(op, cur_para_no)
            ...
            try:
                ...
                line_no = inspect.findsource(op)[1] + 1 # the returned line number is right before `def`
            except:
                pass
    ...
    def execute(self):
        ...
        if len(self._cmd) - 1 > len(para_names):
            return SymbolListSearcher([
                Symbol("Syntax Error", f"function signature: {op.__name__ + str(sig)}")
            ])
        ...
        for para_no, argument in enumerate(self._cmd[1:]):
            ...
            if para_type == inspect._empty or isinstance(argument, para_type):
                ...
                try:
                    ...
                    raise TypeError(f"Parameter {para_name}(no {para_no}): Can't convert {argument} to type {para_type}")
            else:
                raise TypeError(f"Parameter {para_name}(no {para_no}): Can't convert {argument} to type {para_type}")
    ...
    def get_intellisense_searcher(self):
        ...
        if len(segs) == 0:
            ...
            if cur_para_no >= len(para_names):
                ...
                return self._package_manager.get_searcher_factory_by_op(op)(op, *self._cmd[1:])
            else:
                para_name = para_names[cur_para_no]
                para_type = sig.parameters[para_name].annotation
                searcher_factory = self._package_manager.get_searcher_factory_by_type(para_type)
